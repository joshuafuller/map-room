...
// A missing element is itself a failure, and it must not hide the checks that follow it.
const step = async (label, run) => {
  try {
    if (!await page.locator("#map-manager").isVisible()) {
      failures.push(`${label}: the map manager was no longer open`);
...
const stripJobs = (route) => route.fetch()
  .then(async (response) => route.fulfill({ response, json: { ...await response.json(), jobs: [] } }));
...
try {
  ...
  await step("library view", async () => {
    ...
    const headings = (await page.locator("#manager-library-view h3").allTextContents()).join(" | ");
    if (headings !== "Installed maps | Build activity") failures.push(`library restated its own title instead of naming only its sections: ${headings}`);
  });

  await step("idle library", async () => {
    await page.route("**/api/maps", stripJobs);
    await page.waitForFunction(() => document.querySelectorAll(".job-row").length === 0);
    if (await page.locator("#jobs-section").isVisible()) failures.push("a library with no build activity still showed an empty build activity section");
    await page.unroute("**/api/maps");
  });

  await step("creation view", async () => {
  ...
  await step("back from an untouched form", async () => {
    await page.locator("#manager-add-map").click();
  ...
  await step("catalog combobox", async () => {
    await page.locator("#manager-add-map").click();
  ...
  await step("region selection", async () => {
  ...
  await step("source workflows", async () => {
  ...
  if (renameTarget) {
    ...
    await step("in-place rename", async () => {
    ...
    await step("rename restored", async () => {
    ...
    await step("canceling a rename", async () => {
  ...
  for (const original of initialMaps) {
    ...
    if (current && current.name !== original.name) {
      await fetch(`${baseUrl}/api/maps/${original.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: original.name }) });
    }
  }
