...
test("generates Cyberpunk ATAK XML for the exact PNG XYZ preview route", () => {
  ...
  assert.match(xml, /https:\/\/maps\.example\.test\/styles\/cyberpunk\/512\/\{\$z\}\/\{\$x\}\/\{\$y\}\.png/);
...
test("generates Cyberpunk Tactical ATAK XML as a separate map source", () => {
  ...
  assert.match(xml, /https:\/\/maps\.example\.test\/styles\/cyberpunk-tactical\/512\/\{\$z\}\/\{\$x\}\/\{\$y\}\.png/);
